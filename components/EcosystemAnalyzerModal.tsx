...
export const EcosystemAnalyzerModal: React.FC<EcosystemAnalyzerModalProps> = ({ onClose }) => {
  ...
  const renderAnalysis = (result: string) => {
    ...
    return result.split('\n').map((line, index) => {
      if (line.startsWith('### ')) {
        return <h3 key={index} className="text-lg font-semibold text-gray-900 dark:text-white mt-4 mb-2">{line.substring(4)}</h3>;
      }
      ...
      if (line.trim().startsWith('|') && line.trim().endsWith('|')) {
        ...
        return (
            <tr key={index} className={isHeader ? 'border-b border-gray-300 dark:border-gray-600' : ''}>
    ...
    .reduce((acc: (React.ReactElement | React.ReactElement[])[], line: React.ReactElement) => {
        ...
        if (Array.isArray(last) && last[0].type === 'table' && line.type === 'tr') {
            ...
            last[0] = React.cloneElement(table, {}, newTbody);
        } else if (line.type === 'tr') {
            acc.push([<table key={acc.length} className="w-full my-4 text-sm table-auto border-collapse border border-gray-300 dark:border-gray-700"><tbody>{line}</tbody></table>]);
        }
        else {
            acc.push(line);
        }
        ...
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
                ...
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Ecosystem Analyzer</h2>
            </div>
          ...
          <div className="w-1/2 p-6 overflow-y-auto">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full">
                  ...
                  <p className="text-gray-800 dark:text-white">Analyzing ecosystem...</p>
              </div>
            )}
            {!isLoading && !analysisResult && (
                <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
                    <CpuChipIcon className="w-16 h-16 mb-4"/>
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white">Analysis Report</h3>
