...
export const FilePreviewModal: React.FC<FilePreviewModalProps> = ({ file, onClose, onEdit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl m-4 transform transition-all flex flex-col" style={{maxHeight: '85vh'}}>
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center shrink-0">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white truncate pr-4">Preview: {file.name}</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white text-2xl leading-none flex-shrink-0">&times;</button>
        </div>

        <div className="p-6 flex-1 overflow-y-auto flex items-center justify-center">
          {canPreviewText ? (
            <pre className="w-full h-full bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-300 p-4 rounded-md whitespace-pre-wrap font-mono text-sm overflow-auto">
            ...
            <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 dark:text-gray-400">
              <DocumentIcon className="w-24 h-24 text-gray-400 dark:text-gray-600 mb-4" />
